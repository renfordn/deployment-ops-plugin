...
**TODO(deployment-ops-plugin, Phase 6a/6b/6c of tasks.md)**: Upstream `agent-creator.md` only
implements a single (Create) mode. This vendored copy is the adaptation source for a dual-mode
`mode: create | validate` split — Create mode (below, largely unchanged from upstream) scaffolds
a new agent file, and as of Phase 5 is implemented as executable Python in
`scripts/create_agent.py` (`create_agent(agents_dir, name, description) -> CreateResult`),
including the name-collision refusal. Validate mode is still net-new behavior (three loops:
tool-grant cross-check, sibling-component cross-check, best-practice-doc conformance) with no
upstream reference implementation and is not yet present in this file. Do not assume Validate
mode exists until a later phase adds it.
...
Validate with: `python scripts/validate_plugin.py .` (TODO(deployment-ops-plugin): upstream
referenced a `scripts/validate-agent.sh` utility that does not exist in this plugin; this will be
superseded by `scripts/validate_plugin.py`, Phase 3 of tasks.md)
